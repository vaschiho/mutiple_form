...
const CheckBoxYear = () => {
    ...
    const [checkedItems, setCheckedItems] = useState([
        { id: 1, name: 'Online Service', period: "year", description: " Access to multiplayer games", price: 1, checked: false },
        { id: 2, name: 'Large Storage', period: "year", description: "Extra 1TB of cloud save", price: 20, checked: false },
        { id: 8, name: 'Customizable Profile', period: "year", description: "Custom theme on your profile", price: 20, checked: false }
    ]);
                ...
                {checkedItems.map(item => (
                    <label key={item.id} className={`flex justify-between  gap-10 border rounded-lg p-3 items-center cursor-pointer border-[#f0f6ff] my-3   hover:border-[#473dff] hover:bg-[#fafbff] ${item.checked ? "bg-[#fafbff] border-[#473dff]" : ""}`}>
                        ...
                        <div className='justify-self-end'>
                            <p className='text-[#473dff] text-sm'>+${item.price}/yr</p>
                        </div>
                    </label>
                ))}
